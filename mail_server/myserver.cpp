...
void serverThread(MyHelper helper, MySocket &serverSocket, int new_socket, string argDir)//argdir is receiver directory
{
   ...
   do
   {
      ...
      if (command == "SEND\n")
      {
         ...
         {
            // write file with data in directory of user who receives the mail
            string fullDir = argDir + '/' + receiver.substr(0, receiver.length() - 1);
            int mailId;

            if (access(fullDir.c_str(), F_OK) == -1)//if directory doesn't exist,make one
            {
            ...
            while (1)
            {
               ...
               {
                  mailId++;
                  filename = fullDir + '/' + to_string(mailId) + '_' + subject.substr(0, subject.length() - 1) + ".txt";//1_statAsk.txt
               }
               else
               {
                  break;
      ...
      {
         string user = serverSocket.recvMessage(new_socket);//whose inbox we are checking
         ...
         else
         {
            ...
            if (mails.size() > 0)//client wants to know what list of mails does user have?or,client's sent mails to user
            {
            ...
            else
            {
               ...
               if (mailIndex < mails.size())
               {
                  ...
                  if (remove(fulldir.c_str()) == 0)
                  {
                     ...
                     serverSocket.sendMessage("deleted\0", new_socket);
                  }
                  else
                  {
